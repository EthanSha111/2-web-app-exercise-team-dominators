...
def search():
    ...
    if request.method == 'POST':
        ...
        search_results = events.find(
            {"event_name": {"$regex": query, "$options": 'i'}})  # Basic text search
...
def delete():
    ...
    if event_id:
        users.update_one({"_id": ObjectId(session.get("id"))}, {
                         "$pull": {"watchlist": ObjectId(event_id)}})
...
def settings_save():
    ...
    print(f"Contact Us Message: {contact_us_message}")
...
def add_to_watchlist():
    ...
    if event_id:
        if users.find_one({"_id": ObjectId(session.get("id")), "watchlist": ObjectId(event_id)}):
            ...
            users.update_one({"_id": ObjectId(session.get("id"))}, {
                             "$push": {"watchlist": ObjectId(event_id)}})
        return redirect(url_for('watchlist'))
    else:
